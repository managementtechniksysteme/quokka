...
class ConstructionReportUpdateRequest extends FormRequest
{
    public function rules(): array
    {
        $construction_report = $this->construction_report;
        ...
        return [
            'services_provided_on' => [
                'required',
                'date',
                Rule::unique('construction_reports')->where(function ($query) use ($services_provided_on, $project_id) {
                    return $query->where('services_provided_on', $services_provided_on)->where('project_id', $project_id);
                })->ignore($construction_report),
            ],
