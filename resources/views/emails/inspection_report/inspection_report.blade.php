...
**UVC Strahler**<br />
**Anzahl, Typ:** {{ Number::format($inspectionReport->uvc_lamp_quantity) }} x {{ $inspectionReport->uvc_lamp_type }}<br />
**Betriebsstunden:** {{ Number::format($inspectionReport->uvc_lamp_operating_hours) }}h<br />
**Impulse:** {{ Number::format($inspectionReport->uvc_lamp_impulses) }}<br />
...
{{ Number::format($inspectionReport->uvc_lamp_uv_intensity_departure) }}{{ $inspectionReport->uvc_lamp_values_unit_string }}<br />
...
**Voralarm:** {{ Number::format($inspectionReport->uvc_sensor_pre_alarm) }}{{ $inspectionReport->uvc_sensor_values_unit_string }}<br />
**Abschaltpunkt:** {{ Number::format($inspectionReport->uvc_sensor_cut_off_point) }}{{ $inspectionReport->uvc_sensor_values_unit_string }}<br />
...
**Wasser**<br />
**Durchfluss:** {{ Number::format($inspectionReport->water_flow_rate) }} m³/h<br />
**minimale, gemessene Transmission[100mm]:**
{{ Number::format($inspectionReport->water_minimum_uv_transmission) }}%,
{{ Number::format($inspectionReport->water_measured_uv_transmission) }}%<br />
