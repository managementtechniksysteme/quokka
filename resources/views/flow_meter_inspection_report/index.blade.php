        ...
        <div class="container pt-4">
            <h3>
                ...
                @unless($flowMeterInspectionReports->isEmpty())
                    <small class="text-muted">{{ trans_choice('messages.entries', $flowMeterInspectionReports->total()) }}</small>
                @endunless
            </h3>

            <div class="scroll-x d-flex">
                @can('create', \App\Models\FlowMeterInspectionReport::class)
                    <a class="btn btn-outline-secondary border-0 d-inline-flex align-items-center" href="{{ route('flow-meter-inspection-reports.create') }}">
    ...
    <div class="container mt-4">
        ...
        <div class="mt-3">
            @forelse ($flowMeterInspectionReports as $flowMeterInspectionReport)
                @component('flow_meter_inspection_report.overview_card', [ 'flowMeterInspectionReport' => $flowMeterInspectionReport, 'actionRedirect' => 'index' ])
                @endcomponent
            ...
            @empty
                <div class="text-center mt-4">
                    ...
                    @else
                        <p class="lead text-muted">Es sind keine Prüfberichte für Durchflussmesseinrichtungen im System vorhanden.</p>
                        @can('create', \App\Models\FlowMeterInspectionReport::class)
                            <p class="lead">Lege einen neuen Prüfbericht an.</p>
                            <a class="btn btn-primary btn-lg d-inline-flex align-items-center" href="{{ route('flow-meter-inspection-reports.create') }}">
