    ...
    <div class="q-container">
        ...
        <div class="q-page-head">
            ...
            @can('create', \App\Models\FlowMeterInspectionReport::class)
                <a class="btn btn-primary text-white d-none d-md-inline-flex align-items-center gap-2" href="{{ route('flow-meter-inspection-reports.create') }}">
            ...
            <div class="d-flex d-md-none align-items-center gap-2">
                @unless($flowMeterInspectionReports->isEmpty())
                    <div class="q-subtitle mb-0">{{ trans_choice('messages.entries', $flowMeterInspectionReports->total()) }}</div>
                @endunless
                @can('create', \App\Models\FlowMeterInspectionReport::class)
                    <a class="btn btn-primary text-white d-inline-flex align-items-center gap-2 ms-auto" style="flex: none;" href="{{ route('flow-meter-inspection-reports.create') }}">
                        <svg class="icon-bs icon-16"><use href="{{ asset('svg/bootstrap-icons.svg') }}#plus"></use></svg>
                        Prüfbericht
                    </a>
                @endcan
        ...
        @unless ($flowMeterInspectionReports->isEmpty() && !Request::get('search'))
            ...
            <div class="d-flex d-md-none align-items-center gap-2 mb-3">
                <form class="flex-grow-1" action="{{ route('flow-meter-inspection-reports.index') }}" method="get">
                    ...
                    <div class="position-relative flex-grow-1">
                        <div class="input-group">
                            ...
                            @endif
                            <button type="button" class="btn q-btn q-btn-icon d-flex align-items-center justify-content-center" data-bs-toggle="offcanvas" data-bs-target="#flowMeterInspectionReportQuickFilterSheet" aria-controls="flowMeterInspectionReportQuickFilterSheet" aria-label="Schnellfilter">
                ...
                <button class="btn q-btn q-btn-icon" type="button" data-bs-toggle="offcanvas" data-bs-target="#flowMeterInspectionReportSortSheet" aria-controls="flowMeterInspectionReportSortSheet" aria-label="Sortierung">
                    <svg class="icon-bs icon-16"><use href="{{ asset('svg/bootstrap-icons.svg') }}#sort-down"></use></svg>
                </button>
            </div>

            @php
                $flowMeterInspectionReportFilterUsername = Auth::user()->username;
                $flowMeterInspectionReportShowFinished = Auth::user()->settings->show_finished_items;
                $flowMeterInspectionReportQuickFilters = [
                    'Meine Prüfberichte' => 't:' . $flowMeterInspectionReportFilterUsername . ($flowMeterInspectionReportShowFinished ? '' : ' !ist:erledigt'),
                    'Meine nicht unterschriebenen Prüfberichte' => 't:' . $flowMeterInspectionReportFilterUsername . ' ist:neu',
                ];
            @endphp
            <div class="offcanvas offcanvas-bottom q-sheet" tabindex="-1" id="flowMeterInspectionReportQuickFilterSheet" aria-label="Schnellfilter">
                ...
                <div class="offcanvas-body">
                    ...
                    @foreach($flowMeterInspectionReportQuickFilters as $quickFilterLabel => $quickFilterExpr)
            ...
            @php
                $flowMeterInspectionReportSortOptions = [
            ...
            @endphp
            <div class="offcanvas offcanvas-bottom q-sheet" tabindex="-1" id="flowMeterInspectionReportSortSheet" aria-label="Sortierung">
                <div class="q-sheet__handle" aria-hidden="true"><span class="q-sheet__handle-bar"></span></div>
                <div class="offcanvas-body">
                    ...
                    <form action="{{ route('flow-meter-inspection-reports.index') }}" method="get">
                        @if(request()->has('search'))
                            <input type="hidden" name="search" value="{{ request()->search ?? '' }}">
                        @endif
                        @foreach($flowMeterInspectionReportSortOptions as $sortValue => $sortMeta)
