...
@endif
@if(count($employees) === 1)
\footnotesize{\textbf{Verfügbarer Urlaub:} {!! Latex::escape(Number::format($employees[0]->holidays)) !!} {!! Latex::escape($holidayService->unit) !!}}
@endif
@if(count($report) > 0)
@if(count($employees) === 1)
...
@if(count($employees) === 1)
\footnotesize{{!! Latex::escape(\Carbon\Carbon::parse($entry->date)->translatedFormat('D d.m.Y')) !!}} & \footnotesize{{!! Latex::escape($entry->project) !!}} & \footnotesize{{!! Latex::escape($entry->amount_hours ? Number::format($entry->amount_hours) : '') !!}} & \footnotesize{{!! Latex::escape($entry->amount_allowances ? Number::format($entry->amount_allowances) : '') !!}} & \footnotesize{{!! Latex::escape($entry->amount_overtime_50 ?Number::format($entry->amount_overtime_50) : '') !!}} & \footnotesize{{!! Latex::escape($entry->amount_overtime_100 ? Number::format($entry->amount_overtime_100) : '') !!}} & \footnotesize{{!! Latex::escape($entry->amount_time_balance ? Number::format($entry->amount_time_balance) : '') !!}} & \footnotesize{{!! Latex::escape($entry->amount_holidays ? Number::format($entry->amount_holidays) : '') !!}} \\
@else
...
@endforeach
@if(count($employees) === 1)
\footnotesize{\textbf{Summe}} & \footnotesize{} & \footnotesize{\textbf{{!! Latex::escape($report->sum('amount_hours') > 0 ? Number::format($report->sum('amount_hours')) : '') !!}}} & \footnotesize{\textbf{@if($report->sum('amount_allowances') > 0){!! Latex::escape(Number::format($report->sum('amount_allowances'))) !!} @if($allowancesService && $allowancesService->costs > 0) \newline ({!! Latex::escape(Number::format($report->sum('amount_allowances') * $allowancesService->costs) . $currencyUnit) !!})@endif @endif}} & \footnotesize{\textbf{{!! Latex::escape($report->sum('amount_overtime_50') > 0 ? Number::format($report->sum('amount_overtime_50')) : '') !!}}} & \footnotesize{\textbf{{!! Latex::escape($report->sum('amount_overtime_100') > 0 ? Number::format($report->sum('amount_overtime_100')) : '') !!}}} & \footnotesize{\textbf{{!! Latex::escape($report->sum('amount_time_balance') > 0 ? Number::format($report->sum('amount_time_balance')) : '') !!}}} & \footnotesize{\textbf{{!! Latex::escape($report->sum('amount_holidays') > 0 ? Number::format($report->sum('amount_holidays')) : '') !!}}} \\
@else
\footnotesize{\textbf{Summe}} & \footnotesize{} & \footnotesize{} & \footnotesize{\textbf{{!! Latex::escape($report->sum('amount_hours') > 0 ? Number::format($report->sum('amount_hours')) : '') !!}}} & \footnotesize{\textbf{@if($report->sum('amount_allowances') > 0){!! Latex::escape(Number::format($report->sum('amount_allowances'))) !!} @if($allowancesService && $allowancesService->costs > 0) \newline ({!! Latex::escape(Number::format($report->sum('amount_allowances') * $allowancesService->costs) . $currencyUnit) !!})@endif @endif}} & \footnotesize{\textbf{{!! Latex::escape($report->sum('amount_overtime_50') > 0 ? Number::format($report->sum('amount_overtime_50')) : '') !!}}} & \footnotesize{\textbf{{!! Latex::escape($report->sum('amount_overtime_100') > 0 ? Number::format($report->sum('amount_overtime_100')) : '') !!}}} & \footnotesize{\textbf{{!! Latex::escape($report->sum('amount_time_balance') > 0 ? Number::format($report->sum('amount_time_balance')) : '') !!}}} & \footnotesize{\textbf{{!! Latex::escape($report->sum('amount_holidays') > 0 ? Number::format($report->sum('amount_holidays')) : '') !!}}} \\
@endif
\hline
\end{longtable}
@else
...
\footnotesize{{!! Latex::escape(Number::format($private_kilometres[array_key_first($private_kilometres)])) !!} ({!! Latex::escape(Number::format($private_kilometres[array_key_first($private_kilometres)] * $kilometre_costs)) . $currencyUnit !!})}
@else
@foreach($private_kilometres as $username => $kilometres)
\footnotesize{{!! Latex::escape(Str::upper($username)) !!}: {!! Latex::escape(Number::format($kilometres)) !!} ({!! Latex::escape(Number::format($kilometres * $kilometre_costs)) . $currencyUnit !!}), }
@endforeach
\footnotesize{Summe: {!! Latex::escape(Number::format(array_sum($private_kilometres))) !!} ({!! Latex::escape(Number::format(array_sum($private_kilometres) * $kilometre_costs)) . $currencyUnit !!})}
@endif
@endif
@if(count($employees) > 1)
\\\\
