@php \Carbon\Carbon::setLocale('de_DE'); @endphp
...
\hline
@endforeach
\footnotesize{\textbf{Summe}} & \footnotesize{} & \footnotesize{\textbf{{!! Latex::escape($report->sum('amount_hours') > 0 ? $report->sum('amount_hours') : '') !!}}} & \footnotesize{\textbf{@if($report->sum('amount_allowances') > 0){!! Latex::escape($report->sum('amount_allowances')) !!} @if($allowancesService && $allowancesService->costs > 0)({!! Latex::escape($report->sum('amount_allowances') * $allowancesService->costs . $currencyUnit) !!})@endif @endif}} & \footnotesize{\textbf{{!! Latex::escape($report->sum('amount_overtime_50') > 0 ? $report->sum('amount_overtime_50') : '') !!}}} & \footnotesize{\textbf{{!! Latex::escape($report->sum('amount_overtime_100') > 0 ? $report->sum('amount_overtime_100') : '') !!}}} & \footnotesize{\textbf{{!! Latex::escape($report->sum('amount_time_balance') > 0 ? $report->sum('amount_time_balance') : '') !!}}} & \footnotesize{\textbf{{!! Latex::escape($report->sum('amount_holidays') > 0 ? $report->sum('amount_holidays') : '') !!}}} \\
\hline
\end{longtable}
@else
