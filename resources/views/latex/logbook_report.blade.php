...
\footnotesize{{!! Latex::escape($entry->vehicle->registration_identifier) !!}} & \footnotesize{{!! Latex::escape($entry->driven_on->translatedFormat('D d.m.y')) !!}} & \footnotesize{{!! Latex::escape(Str::upper($entry->employee->user->username)) !!}} &  \footnotesize{{!! Latex::escape(optional($entry->project)->short_name ?? '') !!}} & \footnotesize{{!! Latex::escape($entry->origin) !!}} & \footnotesize{{!! Latex::escape($entry->destination) !!}} & \footnotesize{{!! Latex::escape(Number::format($entry->start_kilometres)) !!}} & \footnotesize{{!! Latex::escape(Number::format($entry->end_kilometres)) !!}} & \footnotesize{{!! Latex::escape(Number::format($entry->driven_kilometres)) !!}} & \footnotesize{{!! Latex::escape($entry->litres_refuelled ? Number::format($entry->litres_refuelled) : '') !!}} \\
\hline
@endforeach
\footnotesize{\textbf{Summe}} & \footnotesize{} & \footnotesize{} & \footnotesize{} & \footnotesize{} & \footnotesize{} & \footnotesize{} & \footnotesize{} & \footnotesize{\textbf{{!! Latex::escape($report->sum('driven_kilometres') > 0 ? Number::format($report->sum('driven_kilometres')) : '') !!}}} & \footnotesize{\textbf{{!! Latex::escape($report->sum('litres_refuelled') > 0 ? Number::format($report->sum('litres_refuelled')) : '') !!}}} \\
\hline
\end{longtable}
\begin{footnotesize}
