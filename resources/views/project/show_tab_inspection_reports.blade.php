...
@section('tab')
    ...
    <div class="mt-3">
        @forelse ($inspectionReports as $inspectionReport)
            @component('inspection_report.overview_card', [ 'inspectionReport' => $inspectionReport, 'secondaryInformation' => 'withoutProject', 'actionRedirect' => 'project' ])
            @endcomponent
        ...
        @empty
            <div class="text-center">
                ...
                @else
                    <p class="lead text-muted">Dem Projekt {{ $project->name }} sind keine Prüfberichte zugeordnet.</p>
                    @can('create', \App\Models\InspectionReport::class)
                        <p class="lead">Lege einen neuen Prüfbericht an.</p>
                        <a class="btn btn-primary btn-lg d-inline-flex align-items-center" href="{{ route('inspection-reports.create', ['project' => $project->id]) }}">
