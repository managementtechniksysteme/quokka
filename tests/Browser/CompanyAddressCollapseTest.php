...
function companyAddressCollapseUser(): User
{
...
test('desktop shows the collapse trigger and hides the offcanvas trigger', function () {
    $this->actingAs(companyAddressCollapseUser());

    $page = visit(route('companies.create'))->on()->desktop();
...
test('mobile shows the offcanvas trigger and hides the collapse trigger', function () {
    ...
    $page = visit(route('companies.create'))->on()->mobile();
